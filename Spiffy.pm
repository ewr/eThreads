...
sub field {
    ...
    my ($args, @values) = do {
        no warnings;
        local *boolean_arguments = sub { (qw(-weak -ro)) };
        local *paired_arguments = sub { (qw(-package -init -key)) };
    ...
    my $key = $args->{-key} || $field;
    my $code = $code{sub_start};
    if ($args->{-init}) {
        my $fragment;
        if ($args->{-ro}) {
            $fragment = $args->{-weak} ? $code{weak_ro_init} : $code{ro_init};
        } else {
            $fragment = $args->{-weak} ? $code{weak_init} : $code{init};
        }
        $code .= sprintf $fragment, $key, $args->{-init}, ($key) x 4;
    }
    $code .= sprintf $code{set_default}, $key, $default_string, $key
      if defined $default;
    if (!$args->{-ro}) {
        $code .= sprintf $code{return_if_get}, $key;
        $code .= sprintf $code{set}, $key;
    }
    $code .= sprintf $code{weaken}, $key, $key 
      if ($args->{-weak} && !$args->{-ro});
    $code .= sprintf $code{sub_end}, $key;
...
in the object. There are several parameters that can be given to alter 
how a field behaves.
...
Pass -init with a string containing a bit of code that should be run to 
initialize the field the first time it is accessed as a get.  If the 
first usage of the field is a set, the init won't be run.

=item -key

Normally the field contents are stored in the object with a key of the 
same name as the field.  So a field 'foo' would have its contents stored 
in $self->{foo}.  If that's not the desired operation, pass -key with a 
string naming the key you would like to use.  So, for instance

    field 'foo' => -key=>'bar';

would be called via $self->foo, but would store its contents in 
$self->{bar}.

=item -package

Pass -package with the name of the package in which to create the accessor, 
if it is different than the current package.

=item -ro

If the -ro argument is given, the accessor will be get only.  This is 
different from a const in that the value is still stored in the object, 
and can be set directly.  The value can still be set, just via the 
object hash:

    field 'foo' => -ro;
    $self->foo('bar')       # does nothing
    $self->{foo} = 'bar'    # sets the field value

=item -weak

If the value of the field is a reference, -weak will cause that ref to 
be weakened using Scalar::Util::weaken.
